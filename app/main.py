...
def get_projects(db: Session = Depends(get_db)):
    ...
    return [{"id": p.id, "title": p.title, "description": p.description, "status": p.status} for p in projects]
...
def register(data: dict, db: Session = Depends(get_db)):
    try:
        ...
        if not role:
            ...
            return {"success": False, "message": f"Роль '{role_code}' не существует", "available_roles": [r.code for r in available_roles]}
        ...
        db.commit()
        db.refresh(new_user)
        return {"success": True, "message": "Регистрация успешна", "user": {"email": new_user.email, "name": new_user.name, "role": new_user.role.code}}
...
def login(data: dict, db: Session = Depends(get_db)):
    try:
        ...
        return {
            "success": True, "message": "Вход выполнен успешно", "token": access_token,
...
def apply_to_task(
    ...
    db: Session = Depends(get_db), current_user: models.User = Depends(volunteer_required)
):
    try:
...
def get_my_applications(db: Session = Depends(get_db), current_user: models.User = Depends(volunteer_required)):
    applications = db.query(models.TaskApplication).filter(models.TaskApplication.user_id == current_user.id).all()
    return [{
        "id": a.id, "task_id": a.task_id, "task_title": a.task.title if a.task else "Неизвестно",
        "status": a.status, "message": a.message, "applied_at": a.applied_at
    } for a in applications]


@app.post("/api/reports/create")
def create_report(data: dict, db: Session = Depends(get_db), current_user: models.User = Depends(volunteer_required)):
    try:
...
def get_my_reports(db: Session = Depends(get_db), current_user: models.User = Depends(volunteer_required)):
    reports = db.query(models.TaskReport).filter(models.TaskReport.user_id == current_user.id).all()
    return [{
        "id": r.id, "assignment_id": r.assignment_id, "comment": r.comment,
        "hours": r.hours, "photo_url": r.photo_url, "is_approved": r.is_approved,
        "submitted_at": r.submitted_at
    } for r in reports]


@app.post("/api/projects/create")
def create_project(data: dict, db: Session = Depends(get_db), current_user: models.User = Depends(organizer_required)):
    try:
...
def create_task(data: dict, db: Session = Depends(get_db), current_user: models.User = Depends(organizer_required)):
    try:
        task = models.Task(
            title=data.get("title"), description=data.get("description"),
...
def edit_task(task_id: int, data: dict, db: Session = Depends(get_db), current_user: models.User = Depends(organizer_required)):
    try:
        task = db.query(models.Task).filter(models.Task.id == task_id).first()
        if not task:
            return {"success": False, "message": "Задача не найдена"}
...
def get_pending_applications(db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    applications = db.query(models.TaskApplication).filter(models.TaskApplication.status == "pending").all()
    return [{
        "id": a.id, "task_id": a.task_id, "task_title": a.task.title if a.task else "Неизвестно",
        "user_id": a.user_id, "user_name": a.user.name if a.user else "Неизвестно",
        "message": a.message, "applied_at": a.applied_at
    } for a in applications]


@app.post("/api/applications/{app_id}/approve")
def approve_application(app_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    try:
        application = db.query(models.TaskApplication).filter(models.TaskApplication.id == app_id).first()
        if not application:
...
def reject_application(app_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    try:
        application = db.query(models.TaskApplication).filter(models.TaskApplication.id == app_id).first()
        if not application:
            return {"success": False, "message": "Заявка не найдена"}
...
def get_pending_reports(db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    reports = db.query(models.TaskReport).filter(models.TaskReport.is_approved == False).all()
    return [{
        "id": r.id, "assignment_id": r.assignment_id, "user_id": r.user_id,
        "user_name": r.user.name if r.user else "Неизвестно", "comment": r.comment,
        "hours": r.hours, "photo_url": r.photo_url, "submitted_at": r.submitted_at
    } for r in reports]


@app.post("/api/reports/{report_id}/approve")
def approve_report(report_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    try:
        report = db.query(models.TaskReport).filter(models.TaskReport.id == report_id).first()
        if not report:
            return {"success": False, "message": "Отчёт не найден"}
        report.is_approved = True
        assignment = db.query(models.TaskAssignment).filter(models.TaskAssignment.id == report.assignment_id).first()
        if assignment:
...
def reject_report(report_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    try:
        report = db.query(models.TaskReport).filter(models.TaskReport.id == report_id).first()
        if not report:
            return {"success": False, "message": "Отчёт не найден"}
...
def get_all_users(db: Session = Depends(get_db), current_user: models.User = Depends(admin_required)):
    ...
    return [{
        "id": u.id, "email": u.email, "name": u.name, "role": u.role.code,
        "role_name": u.role.name, "is_active": u.is_active, "created_at": u.created_at
    } for u in users]


@app.post("/api/admin/users/{user_id}/toggle-active")
def toggle_user_active(user_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(admin_required)):
    try:
...
def change_user_role(user_id: int, data: dict, db: Session = Depends(get_db), current_user: models.User = Depends(admin_required)):
    try:
        user = db.query(models.User).filter(models.User.id == user_id).first()
        if not user:
            return {"success": False, "message": "Пользователь не найден"}
...
def admin_stats(db: Session = Depends(get_db), current_user: models.User = Depends(admin_required)):
    ...
    return {
        "total_users": total_users, "active_users": active_users, "roles": roles_stats,
        "tasks": tasks_by_status, "projects": db.query(models.Project).count(),
...
async def upload_document(doc_type: str, file: UploadFile = File(...), db: Session = Depends(get_db), current_user: models.User = Depends(volunteer_required)):
...
def verify_document(doc_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
...
def get_analytics_summary(db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    from sqlalchemy import func
    avg_hours = db.query(func.avg(models.TaskReport.hours)).filter(models.TaskReport.is_approved == True).scalar() or 0
    return {
...
def create_feedback(project_id: int, feedback: FeedbackCreate, db: Session = Depends(get_db), current_user: models.User = Depends(volunteer_required)):
...
def direct_assign(task_id: int, user_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(curator_required)):
    task = db.query(models.Task).filter(models.Task.id == task_id).first()
    if not task:
    ...
    assignment = models.TaskAssignment(
        task_id=task_id, user_id=user_id, assigned_by=current_user.id, status="assigned"
    )
    db.add(assignment)
    db.commit()
...
async def create_report_with_photos(
    ...
    photos: list[UploadFile] = File(None), db: Session = Depends(get_db),
    ...
    report = models.TaskReport(
        assignment_id=assignment.id, user_id=current_user.id, comment=comment,
        hours=hours, photo_url=",".join(photo_urls) if photo_urls else None, is_approved=False
    )
    db.add(report)
    db.commit()
...
from app.routers import checkins, tasks_extra, reports_router, projects_api, admin, stats
